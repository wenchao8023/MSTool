...
+(UIButton*)createButtonWithFrame:(CGRect)frame ImageName:(NSString*)imageName Target:(id)target Action:(SEL)action Title:(NSString*)title
{
    ...
    //取消button下的线
    if (title != nil) {
        [button cancelUndenlineWithText:title];
    }
...
+(UIBarButtonItem *)createNaviBackButtonTarget:(id)target Action:(SEL)action {
    ...
    return [[UIBarButtonItem alloc] initWithCustomView:[self createButtonWithFrame:CGRectMake(0, 0, 44, 44) ImageName:@"backLeftWhite.png" Target:target Action:action Title:nil]];
}
...
+(UIView *)createHeadViewWithTitle:(NSString *)title Target:(id)target Action:(SEL)action leftTag:(NSInteger)leftTag leftImage:(NSString *)leftImage rightTag:(NSInteger)rigthTag rightImage:(NSString *)rightImage {
    ...
    UIView *headView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREENW, NAVIGATIONBAR_HEIGHT)];
    
    headView.backgroundColor = headViewBackColor;
    
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((SCREENW - 100) / 2, 20, 100, 40)];
    ...
    rightBtn.frame = CGRectMake(SCREENW - 60, 20, 40, 40);
    
    rightBtn.tag = rigthTag;
...
/*
 这几句代码可以得到当前时间，currentTime返回的是一个长度为19的字符串,
 例如:”2016-01-21 09-46-58” HH 表示24进制
 拿到这个时间字符串就可以做一些关于时间的操作了
 */
+(NSString *)stringFromDateWithcomplete:(NSDate *)date
{
