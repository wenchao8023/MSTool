...
#import "SmartCommunicationManager.h"
...
@interface SmartCommunicationManager ()<GCDAsyncSocketDelegate>
...
@implementation SmartCommunicationManager
...
- (void)smartConfigWithSSID:(NSString *)ssid pswd:(NSString *)pswd {
...
-(void)createSocketToReset {
    ...
    [self createSocketWithDelegate:self andHost:TCP_HOST andPort:TCP_PORT];
}
...
- (void)socketWriteDataWithData:(NSData *)requestData andTag:(long)tag {
    ...
    [self.socketManager socketWriteData:requestData andTag:tag];
}
...
-(void)analysisData:(NSData *)data {
    ...
    if (cmd == CMD_HANDSHAKE_R) {   // 握手返回
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.cmdConfig setDicWithCMD:cmd andValue:[self.dataManager getValueWithData:data]];
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (cmd == CMD_GET_playAlbum_R) {   // 播放列表 或收藏列表
                ...
                for (int i = 0; (i + 1) * length <= data.length; i++) {
                    ...
                    [self.cmdConfig setAlbumWithCMD:cmd errNo:errNo dic:tempDic];
                }
            ...
            [self.cmdConfig setDicWithCMD:cmd andDic:getDataDic];
        });
    }
...
-(void)handShake {
    ...
    [self socketWriteDataWithData:[self.dataManager getHandShakeData] andTag:GET_HANDSHAKE_TAG];
}

//  发送路由信息
-(void)sendRouteDataWithSSID:(NSString *)ssid andPassword:(NSString *)password {
    
    NSData *requestData = [self.dataManager getRouterInfoDataWithSSID:ssid andPassword:password];
    
    [self socketWriteDataWithData:requestData andTag:SEND_ROUTE_TAG];
}
...
//    
//    [self.socketManager connectSocketWithDelegate:self withHost:[tempDic objectForKey:@"IP"] andPort:[[tempDic objectForKey:@"PORT"] integerValue]];
}
